...
const prepareAnimation = () => {
  ...
  currentSectionId.value = Math.min(
    triggerCount,
    triggerSections.value.length - 1,
  );
  ...
  if (triggerCount > triggerSections.value.length - 1) {
    ...
    triggerCount <= triggerSections.value.length - 1 &&
    currentScrollTop <= currentOffsetTop
  ) {
    const min =
      triggerCount == 0 ? 0 : triggerSections.value[triggerCount - 1].offsetTop;
    const max = currentOffsetTop - min;
    sectionPercent.value = Math.min(
      ((max - currentScrollTop) * 100) / max,
      100,
    );
  } else if (
    triggerCount <= triggerSections.value.length - 1 &&
  ...
  >
    ...
    <FollowingFrame v-if="frameSectionRef" :contentSection="frameSectionRef">
      <div ref="topRef" class="h-full w-full bg-white p-[3dvw]">
        <div
          class="h-full w-full relative flex justify-center items-center rounded-4xl overflow-hidden"
        >
          <div
            class="absolute h-full w-full top-0 left-0 bg-[url('/backgrounds/about_me_subtle.png')] bg-cover bg-bottom"
          ></div>
          <div class="h-[80%] w-full overflow-hidden px-[3dvw] z-10">
            <div class="absolute h-6 z-30 flex items-center gap-2 text-neutral-900">
              <span class="w-10">{{ stories[currentSectionId].from }}</span>
              <div class="h-full w-40 flex items-center border-x border-neutral-900/10">
                <div
                  class="h-1 bg-neutral-900"
            ...
            <div
              ref="contentRef"
              class="h-full w-full transition-transform duration-400 text-neutral-900"
            >
              <div
