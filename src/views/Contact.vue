...
const handleScroll = () => {
  const relativeScrollPosition =
    frameRef.value.sectionRef.getBoundingClientRect().top;
  translationRef.value = Math.round(
    relativeScrollPosition + paddingTopRef.value * 1.5,
  );
};

onMounted(() => {
  paddingTopRef.value = parseFloat(
    getComputedStyle(contactSectionRef.value).paddingTop,
  );
  ...
  <FramedMainSection ref="frameRef" id="contact" class="h-dvh">
    <Parallax>
      ...
      >
        ...
        >
          <div
            class="h-full w-full brightness-[.85] bg-[url('/backgrounds/contact_bg.png')] bg-cover"
          ></div>
        </div>
        <div
